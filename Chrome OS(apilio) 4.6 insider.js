// ==UserScript==
// @name         网页模拟桌面系统 (Web OS) - 终极优化版(apilio) 4.6 Insider
...
function getTheme() {
    ...
    function showBootAnimation(callback) {
        ...
        skipHint.style.cssText = 'position:absolute;top:16px;right:16px;font-family:system-ui,sans-serif;font-size:12px;color:rgba(255,255,255,0.4);letter-spacing:0.5px;z-index:2147483648;user-select:none;pointer-events:none;';
        ...
        // Phase 1: 光晕渐显 (0.3s后)
        _skipTimers.push(setTimeout(function() {
        ...
        // ---- Esc 跳过监听 ----
        document.addEventListener('keydown', function _onBootKey(e) {
            if (e.key === 'Escape') {
                document.removeEventListener('keydown', _onBootKey);
                _doSkip();
            }
        });
    }
    ...
    function showShutdownAnimation(callback) {
        ...
        function _doSkip() {
            ...
            _skipped = true;
            _skipTimers.forEach(function(id) { clearTimeout(id); });
            try { shutdownAudio.pause(); shutdownAudio.currentTime = 0; } catch(e) {}
            try { shutdownOverlay.remove(); } catch(e) {}
            try { animStyle.remove(); } catch(e) {}
            if (callback) callback();
        }
        ...
        skipHint.textContent = 'Esc → 跳过';
        skipHint.style.cssText = 'position:absolute;top:16px;right:16px;font-family:system-ui,sans-serif;font-size:12px;color:rgba(255,255,255,0.4);letter-spacing:0.5px;z-index:2147483648;user-select:none;pointer-events:none;';
        shutdownOverlay.appendChild(skipHint);
        ...
        // ---- Esc 跳过监听 ----
        document.addEventListener('keydown', function _onShutdownKey(e) {
            if (e.key === 'Escape') {
                document.removeEventListener('keydown', _onShutdownKey);
                _doSkip();
            }
        });
    }
