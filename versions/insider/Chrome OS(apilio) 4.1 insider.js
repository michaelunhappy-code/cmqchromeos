// ==UserScript==
// @name         网页模拟桌面系统 (Web OS) - 终极优化版(apilio) 4.1 Insider
...
(function() {
    ...
    if (_fail >= 5 && Date.now() >= _lockUntil) {
        // 锁定时间已过，重置错误计数
        _fail = 0;
        localStorage.setItem('OS_FAIL', '4');
...
function getTheme() {
    ...
    addRow('✂️ 剪切', function() {
        _clipboard = { action: 'cut', ids: [file.id] };
        el.style.opacity = '0.5';
        const orig = el.querySelector('.file-info-part span:last-child');
        const t = orig.textContent; orig.textContent = '✂️ 已剪切'; setTimeout(function() { orig.textContent = t; }, 1000);
    ...
    addRow('📌 粘贴', function() {
        ...
        // 如果当前文件是文件夹，粘贴到该文件夹内
        if (file.type === 'folder') {
            const count = pasteFiles(file.id);
            if (count > 0) alert('✅ 已粘贴 ' + count + ' 个项目到 ' + file.name);
        } else {
            // 粘贴到与当前文件同级的目录
            const parentId = getParentFolderId(file.id);
            const count = pasteFiles(parentId);
    ...
    function createFolderNode(folder, depth) {
        ...
        header.addEventListener('click', (e) => {
            ...
            if (isExpanded) { const existing = shadow.getElementById('win-runtime-' + folder.id); if (existing) { existing.querySelector('.os-window-header span').click(); } else { openAppWindow(folder.id); } }
        ...
        header.addEventListener('contextmenu', function(e) {
            ...
            addRow('📂 打开', function() { openAppWindow(folder.id); });
            addRow('✏️ 重命名', function() {
                const n = prompt('重命名:', folder.name);
                if (n && n.trim()) { folder.name = n.trim(); saveSystemData(); renderSidebarFiles(); }
            });
            addRow('📋 复制', function() {
            ...
            addRow('📌 粘贴', function() {
                ...
                const count = pasteFiles(folder.id);
                if (count > 0) alert('✅ 已粘贴 ' + count + ' 个项目到 ' + folder.name);
            });
            addRow('🗑️ 删除', function() {
                if (confirm('确定删除文件夹 [' + folder.name + '] 及其所有内容吗？')) {
                    if (folder.children) {
                        folder.children.forEach(childId => {
                            OS_DATA.files = OS_DATA.files.filter(f => f.id !== childId);
                        });
                    ...
                    OS_DATA.files.forEach(f => {
                        if (f.type === 'folder' && f.children) {
                            f.children = f.children.filter(c => c !== folder.id);
                        }
                    });
                    saveSystemData(); renderSidebarFiles();
                }
        ...
        header.addEventListener('drop', function(e) {
            ...
            // 防止拖到同一个文件夹（检查文件是否已在该文件夹内）
            if (folder.children && folder.children.includes(fileId)) return;
            // 防止拖拽文件夹到自身子文件夹中（循环）
            if (file.type === 'folder' && isDescendant(fileId, folder.id)) return;
            // 从原父文件夹的children中移除
    ...
    addMenuItem('📌 粘贴', function() {
        if (!_clipboard.action || _clipboard.ids.length === 0) {
            alert('剪贴板为空！'); return;
        }
        const count = pasteFiles(null); // null 表示粘贴到根目录
        if (count > 0) alert('✅ 已粘贴 ' + count + ' 个项目到根目录');
    });
...
if (target) {
    ...
    if (confirm('将 [' + dragFile.name + '] 移入 [' + targetFile.name + ']？')) {
        ...
        if (!dragFile) return;
        const oldParentId = getParentFolderId(fileId);
        if (!oldParentId) return; // 已经在根目录，不需要操作
        if (confirm('将 [' + dragFile.name + '] 移到根目录？')) {
            const oldParent = OS_DATA.files.find(f => f.id === oldParentId);
            if (oldParent && oldParent.children) {
                oldParent.children = oldParent.children.filter(c => c !== fileId);
            }
            saveSystemData(); renderSidebarFiles();
        }
    }
    ...
    function openAppWindow(fileId, restoreState) {
        ...
        const bodyContainer = win.querySelector('.os-window-body');
            ...
            // 构建路径栏
            let pathSegments = getFilePath(file.id);
            let pathBarHTML = '<span class="explorer-path-seg" data-path-id="root">F://WORKSPACE EXPLORER</span>';
            pathSegments.forEach(function(seg, idx) {
            pathBarHTML += '<span class="explorer-path-sep">//</span>';
            let isCurrent = (idx === pathSegments.length - 1);
    ...
    if ((e.ctrlKey || e.metaKey) && e.key === 'r') {
        ...
        renderExplorer();
                        ...
                        showExplorerContextMenu(e.clientX, e.clientY, [
                            ...
                            { label: '🗑️ 删除', action: function() {
                                if (confirm('确定删除 [' + childFile.name + ']？')) {
                                    ...
                                    navFile.children = navFile.children.filter(function(c) { return c !== childId; });
            ...
            explorerContent.addEventListener('contextmenu', function(e) {
                ...
                const count = pasteFiles(navFile.id);
                if (count > 0) {
                    alert('✅ 已粘贴 ' + count + ' 个项目到 ' + file.name);
                    renderExplorer();
                }
            }},
            { label: '📋 复制文件', action: function() {
                ...
                const count = pasteFiles(file.id);
                if (count > 0) {
                    alert('✅ 已复制 ' + count + ' 个项目到 ' + file.name);
                    renderExplorer();
                }
                }}
                ]);
...
win.querySelector('#btn-download-all').onclick = () => {
    ...
    function makeZip(filesData) {
        let entries = [];
        let offset = 0;

        for (let i = 0; i < filesData.length; i++) {
            let f = filesData[i];
            let nameBytes = new TextEncoder().encode(f.name);
            let contentBytes = new TextEncoder().encode(f.content);
        ...
        // 本地文件头 + 文件内容
        for (let e of entries) {
            write32(0x04034B50); // 本地文件头签名
            write16(20); // 版本
            write16(0); // 标志
            write16(0); // 压缩方式（存储）
            write16(0); // 修改时间
            write16(0); // 修改日期
            write32(e.crc); // CRC-32
            write32(e.size); // 压缩大小
            write32(e.size); // 原始大小
            write16(e.name.length); // 文件名长度
            write16(0); // 额外字段长度
            bytes.set(e.name, pos); pos += e.name.length;
            bytes.set(e.content, pos); pos += e.content.length;
        }

        let centralStart = pos;

        // 中央目录
        for (let e of entries) {
            write32(0x02014B50); // 中央目录头签名
            write16(20); // 版本
            write16(20); // 版本
            write16(0); // 标志
            write16(0); // 压缩方式
        ...
        // 结束记录
        write32(0x06054B50); // 结束签名
        write16(0); // 磁盘号
        write16(0); // 中央目录起始磁盘
        write16(entries.length); // 条目数
        write16(entries.length); // 总条目数
        write32(centralEnd - centralStart); // 中央目录大小
        write32(centralStart); // 中央目录偏移
        write16(0); // 注释长度
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        if (type === 'xlsx') name = name.replace('.xlsx', '.csv');
        else if (type === 'docx') name = name.replace('.docx', '.html');

        filesData.push({ name: name, content: content });
    }

    alert('开始生成ZIP...');

    let blob = makeZip(filesData);

    alert('ZIP生成成功！大小: ' + blob.size + ' bytes');

    let reader = new FileReader();
    reader.onload = function() {
        let base64 = reader.result.split(',')[1];
        let dataUri = 'data:application/zip;base64,' + base64;
        let fileName = 'WebOS_AllFiles_' + new Date().toISOString().slice(0,10) + '.zip';
            ...
            try {
                GM_download({
                    ...
                    onerror: function(err) {
                        fallbackDownload(dataUri, fileName);
                    }
                });
                return;
            } catch(e) {}
