// ==UserScript==
// @name         网页模拟桌面系统 (Web OS) - 终极优化版(apilio) 3.0 稳定版
...
(function() {
    ...
    let DEFAULT_OS_DATA = {
        ...
        settings: {
            ...
            theme: 'default', rememberWindows: false, autoRestoreDesktop: false
        },
        openWindows: [],
        desktopActive: false,
        aiConversations: []
    ...
    function renderMarkdown(text) {
        ...
        html = html.replace(/^### (.+)$/gm, '<h3 style="font-size:1.1em;margin:10px 0 4px;font-weight:bold;">$1</h3>');
        html = html.replace(/^## (.+)$/gm, '<h2 style="font-size:1.2em;margin:12px 0 4px;font-weight:bold;">$1</h2>');
        html = html.replace(/^# (.+)$/gm, '<h1 style="font-size:1.4em;margin:14px 0 6px;font-weight:bold;">$1</h1>');
        ...
        html = html.replace(/^&gt; (.+)$/gm, '<blockquote style="border-left:3px solid var(--accent-color);margin:8px 0;padding:4px 12px;color:var(--text-secondary);">$1</blockquote>');
        html = html.replace(/\n/g, '<br>');
        html = html.replace(/<pre([^>]*)>([\s\S]*?)<\/pre>/g, (m, attrs, code) => {
    ...
    desktop.innerHTML = `
        ...
        <div id="os-sidebar">
            ...
            <div class="sidebar-folder-view">
                ...
                <div class="folder-node">📂 My Code</div>
    ...
    function enterDesktopMode() { desktop.style.display = 'block'; entryBtn.style.display = 'none'; OS_DATA.desktopActive = true; saveSystemDataNow(); if (OS_DATA.settings.rememberWindows && OS_DATA.openWindows && OS_DATA.openWindows.length > 0) restoreWindows(); }
    function exitDesktopMode() { desktop.style.display = 'none'; entryBtn.style.display = 'block'; OS_DATA.desktopActive = false; saveSystemDataNow(); }
    ...
    if (OS_DATA.desktopActive && OS_DATA.settings.autoRestoreDesktop) { requestAnimationFrame(() => { enterDesktopMode(); }); }
    ...
    shadow.getElementById('global-search').addEventListener('input', (e) => { const keyword = e.target.value.toLowerCase().trim(); if (keyword === '对话' || keyword === 'ai') { openAIChatWindow(); e.target.value = ''; shadow.querySelectorAll('.file-node').forEach(node => node.style.display = 'flex'); return; } shadow.querySelectorAll('.file-node').forEach(node => { const fileName = node.querySelector('.file-info-part span:last-child').innerText.toLowerCase(); node.style.display = fileName.includes(keyword) ? 'flex' : 'none'; }); });
    ...
    function renderSidebarFiles() {
        ...
        OS_DATA.files.forEach(file => {
            const el = document.createElement('div'); el.className = 'file-node';
            let icon = '📄'; if (file.type === 'cpp') icon = '⚙️'; if (file.type === 'xlsx') icon = '📊'; if (file.type === 'docx') icon = '📘'; if (file.type === 'md') icon = '📝';
            ...
            el.addEventListener('click', (e) => { if (e.target.classList.contains('file-raw-delete-btn')) return; e.stopPropagation(); openAppWindow(file.id); });
            el.querySelector('.file-raw-delete-btn').addEventListener('click', (e) => { e.stopPropagation(); e.preventDefault(); if (confirm(`确定要彻底删除文件 [${file.name}] 吗？`)) { OS_DATA.files = OS_DATA.files.filter(f => f.id !== file.id); OS_DATA.openWindows = OS_DATA.openWindows.filter(w => w.id !== file.id); saveSystemData(); renderSidebarFiles(); const targetWindow = shadow.getElementById(`win-runtime-${file.id}`); if (targetWindow) targetWindow.remove(); } });
    ...
    function bindWindowDragAndResize(win, header, resizeHandle, extraData) {
        ...
        document.addEventListener('mousemove', (e) => { if (isDragging && !win.classList.contains('maximized')) { let left = e.clientX - shiftX; let top = e.clientY - shiftY; if (top < 0) top = 0; win.style.left = left + 'px'; win.style.top = top + 'px'; if (extraData) saveWindowState(win.id, extraData); } if (isResizing && !win.classList.contains('maximized')) { win.style.width = Math.max(450, startW + (e.clientX - startX)) + 'px'; win.style.height = Math.max(300, startH + (e.clientY - startY)) + 'px'; if (extraData) saveWindowState(win.id, extraData); } });
    ...
    function bindWindowControls(win, title, extraData) {
        ...
        win.querySelector('.win-btn-min').addEventListener('click', (e) => { e.stopPropagation(); win.classList.add('minimized'); const minItem = document.createElement('div'); minItem.className = 'taskbar-minimized-item'; minItem.setAttribute('data-win-id', win.id); minItem.innerHTML = `<span>${title}</span>`; minItem.addEventListener('click', () => { win.classList.remove('minimized'); win.style.zIndex = ++zIndexCounter; const item = taskbarMinList.querySelector(`[data-win-id="${win.id}"]`); if (item) item.remove(); }); taskbarMinList.appendChild(minItem); });
    ...
    // --- C++ Snippets ---
    const CPP_SNIPPETS = [
        { name: 'main', prefix: 'main', body: '#include <iostream>\nusing namespace std;\n\nint main() {\n    $0\n    return 0;\n}' },
    ...
    function openAppWindow(fileId, restoreState) {
        ...
        else { const offset = (zIndexCounter % 8) * 20; win.style.top = `${80 + offset}px`; win.style.left = `${280 + offset}px`; }
        win.innerHTML = `
            <div class="os-window-header">
                <span>${file.name}</span>
                <div class="window-controls-group">
        ...
        if (file.type === 'md') {
            ...
            if (isCpp) {
                ...
                snippetDropdown.innerHTML = snippetHTML; snippetBtn.parentElement.appendChild(snippetDropdown);
            ...
            tableHTML += `</table>`;
            excelUI.innerHTML = `<div class="excel-ribbon"><span>开始</span></div><div class="excel-formula-bar"><span style="font-weight:bold;color:#107c41;">fx</span><input type="text" class="excel-formula-input" id="f-bar-input" placeholder="单元格编辑..."></div><div class="excel-grid-viewport">${tableHTML}</div>`;
    ...
    function openSettingsPanel() {
        ...
        renderModelOptions();
        win.querySelectorAll('.theme-swatch').forEach(swatch => { swatch.addEventListener('click', () => { OS_DATA.settings.theme = swatch.getAttribute('data-theme'); saveSystemData(); applyTheme(); win.querySelectorAll('.theme-swatch').forEach(s => s.classList.remove('active')); swatch.classList.add('active'); }); });
